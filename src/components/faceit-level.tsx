...
const faceitLevelVariants = cva(
  "inline-flex items-center justify-center font-semibold rounded-full text-center cursor-default text-white relative",
  {
    variants: {
      levelVariant: {
        1: "bg-[#121212] border-2 border-[#CDCDCD] border-opacity-10",
        2: "bg-[#121212] border-2 border-[#CDCDCD] border-opacity-10",
        3: "bg-[#121212] border-2 border-[#CDCDCD] border-opacity-10",
        4: "bg-[#121212] border-2 border-[#CDCDCD] border-opacity-10",
        5: "bg-[#121212] border-2 border-[#CDCDCD] border-opacity-10",
        6: "bg-[#121212] border-2 border-[#CDCDCD] border-opacity-10",
        7: "bg-[#121212] border-2 border-[#CDCDCD] border-opacity-10",
        8: "bg-[#121212] border-2 border-[#CDCDCD] border-opacity-10",
        9: "bg-[#121212] border-2 border-[#CDCDCD] border-opacity-10",
        10: "bg-[#121212] border-2 border-[#CDCDCD] border-opacity-10",
        11: "bg-[#121212] border-2 border-[#CDCDCD] border-opacity-10",
      },
      size: {
...
const getFaceitLevelClass = (level: number) => {
  if (level < 1) return 1;
  if (level > 11) return 11;
  return level;
};

const getFaceitLevelData = (level: number) => {
  const levelData: Record<
    number,
    { color: string; segmentPath: string; numberPath: string | null }
  > = {
    1: {
      color: "#EEE",
      segmentPath:
    ...
    7: {
      ...
      numberPath: "M12.546 9.906H9.9v-.958h4v.84L11.807 14.4h-1.36l2.1-4.494z",
    },
    8: {
      color: "#FF6309",
      segmentPath:
...
export interface FaceitLevelProps
  extends React.HTMLAttributes<HTMLDivElement>,
    VariantProps<typeof faceitLevelVariants> {
  level: number;
  size?: "sm" | "md" | "lg";
}

const FaceitLevel = React.forwardRef<HTMLDivElement, FaceitLevelProps>(
  ({ className, level, size = "md", ...props }, ref) => {
    const levelClass = getFaceitLevelClass(level);
    const levelData = getFaceitLevelData(levelClass);
      ...
      >
        ...
        >
          ...
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d={levelData.segmentPath}
            fill={levelData.color}
          />
          {levelData.numberPath && (
            <path d={levelData.numberPath} fill={levelData.color} />
          )}
        </svg>
      </div>
    );
...
FaceitLevel.displayName = "FaceitLevel";

export { FaceitLevel, faceitLevelVariants };
